...
Relatório da quantidade de participantes por tipo de evento | SELECT a.nomemorador AS "Nome do Morador" , Tipo_evento, COUNT(1) "Participação" from tbMorador a JOIN tbMoradorEvento b ON a.id_Morador = b.id_Morador JOIN tbEventos c on c.id_Evento = b.id_Evento GROUP by NomeMorador, Tipo_evento ORDER BY a.NomeMorador
